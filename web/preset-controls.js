(() => {
  ...
  function currentSettings() {
    return {
      ...
      volumeWeight, layoutProfile, uiFontScale, lyricOffset
    };
  }
  ...
  const trackedSelector = [
    ...
    "[data-layout-profile-choice]", "#lyric-offset", "#ui-font-size"
  ].join(",");
  settingsPanel?.addEventListener("click", (event) => {
    if (event.target.closest(trackedSelector)) setTimeout(updateDirty, 0);
  });
  settingsPanel?.addEventListener("input", (event) => {
    if (event.target.matches("#lyric-offset,#ui-font-size,#background-color-picker")) setTimeout(updateDirty, 0);
  });
