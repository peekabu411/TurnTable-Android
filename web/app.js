...
function readLyricFontSizes() {
  try {
    ...
    return Object.fromEntries(Object.keys(lyricStyleNames).map((style) => [style, Math.max(20, Math.min(100, Math.round((Number(saved[style]) || 100) / 5) * 5))]));
...
function applyLyricFontSize(value) {
  lyricFontSizes[lyricStyle] = Math.max(20, Math.min(100, Math.round(Number(value) / 5) * 5));
